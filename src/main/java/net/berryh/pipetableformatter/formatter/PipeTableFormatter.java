/*
	Original source: https://github.com/Jurrie/pipe-table-formatter
	It has been copied and adapted to work better within the IntelliJ Platform environment.
 */
...
public class PipeTableFormatter
{
	...
	/**
	 * The StringBuilder we should output to.
	 */
	private StringBuilder stringBuilder;

	public PipeTableFormatter()
	{
		rows = new ArrayList<>();
	...
	{
		input.lines().forEachOrdered(this::readLine);
		dumpTable();
		final String formattedOutput = stringBuilder.toString();
		stringBuilder = new StringBuilder();
		final String chompedOutput = input.endsWith("\n") ? formattedOutput : StringUtils.chomp(formattedOutput);
		return StringUtil.convertLineSeparators(chompedOutput);
	}

	private void readLine(@Nonnull final String line)
	{
	...
	private void addTableRow(@Nonnull final String line)
	{
	...
	private void updateColumnWidthForCell(@Nonnull final String cell, final int columnIndex)
	{
	...
	private void dumpTableRow(@Nonnull final List<String> row)
	{
	...
	private void dumpTableCell(final List<String> row, final int columnIndex)
	{
		...
		stringBuilder.append(" ".repeat(Math.max(0, columnWidth.get(columnIndex) - cellContent.length())));
	}
}